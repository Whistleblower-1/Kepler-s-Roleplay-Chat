...
--[[
    Roleplay Chat Mod - Updated for Build 42
    Original by buffy (github.com/buffyuwu/PZ-roleplaychat)
    B42 update maintains compatibility with core chat functions while ensuring
    proper SandboxVars access and event handling.
    
    Commands: /me, /melong, /melow, /do, /dolong, /dolow, /name, /low, /whisper, /whisperme, /looc
]]--
...
local function getSandboxVar(varName, default)
    if SandboxVars and SandboxVars.RoleplayChat and SandboxVars.RoleplayChat[varName] ~= nil then
        return SandboxVars.RoleplayChat[varName]
    end
    return default
end
ISChat.sayRange = getSandboxVar("sayRange", 15)
ISChat.meRange = getSandboxVar("meRange", 25)
ISChat.meLongRange = getSandboxVar("meLongRange", 40)
ISChat.lowRange = getSandboxVar("lowRange", 5)
ISChat.whisperRange = getSandboxVar("whisperRange", 2)
...
function ISChat:onGearButtonClick()
    ...
    if getSandboxVar("characterCustomization", true) then
...
function ISChat:onCommandEntered()
    ...
    if not commandProcessed then
        ...
        elseif chatStreamName == "faction" then
            ...
            if getSandboxVar("RadioAlert", true) then
        ...
        elseif chatStreamName == "name" then
            if getSandboxVar("ToggleNameChange", false) then
        ...
        elseif chatStreamName == "looc" then
            if getSandboxVar("ToggleLOOC", false) and not isAdmin() then
        ...
        elseif chatStreamName == "general" then
            if getSandboxVar("ToggleOOC", false) and not isAdmin() then
...
ISChat.addLineInChat = function(message, tabID)
    ...
    local messageRange = getSandboxVar("sayRange", 15)
    local norange
    ...
    if playerAuthor and modPlayerobj then
        ...
        if not norange then
            ...
            elseif string.match(line, "%[Long%]") then
                messageRange = getSandboxVar("meLongRange", 50)
            elseif string.match(line, get_rpname_specific(playerAuthor).." shouts%,") then
                messageRange = getSandboxVar("shoutRange", 50)
            end
